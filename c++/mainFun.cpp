...
long int logPowMod(int number, int degree, int mod)
{
    ...
    long int result = 1;    // a^0 -> a^degree
    long int mult = number; //(a^1, a^2, a^4 ... a^(2^k))
...
vector<int> encrypt(const string &plain_text, long int secretAlisaKey, int mod)
{
...
// для дешифровки
string decrypt(vector<int> &ciphertext, long int secretBobKey, int mod)
{
...
long int modInverse(long int a, int numberAski)
{
    int m0 = numberAski;
    int y = 0, x = 1;
...
long int fastPowMod(int number, int degree, int mod) // при mod = 10 найдем остаток
{                                                    // основан на половине степени
    if (degree == 0)
        return 1;

    long int result = fastPowMod(number % mod, degree / 2, mod) % mod;
